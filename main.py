...
class ProductAPI(Resource):
    def get(self, product_id: str|None=None):
        if product_id:
            ...
            product = db_actions.get_products()
            response = jsonify(product)
...
class ReviewAPI(Resource):
    def rew_db_to_json(self, review_id):
        data = []
        ...
        data_json.status_code = 202
        return data_json
    
    def get(self, review_id: str|None=None):
        if review_id:
            review = db_actions.get_review(review_id)
            return self.rew_db_to_json([review])
        else:
            reviews = db_actions.get_review()
            return self.rew_db_to_json(reviews)
    ...
    def post(self):
        ...
        response = jsonify(dict(review=review_id))
        response.status_code = 201
        return response
